...
const NAV_ITEMS: { href: string; labelKey: TranslationKey; emoji: string }[] = [
  ...
  { href: "/world-cup", labelKey: "nav.worldCup", emoji: "WC" },
...
export function Nav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--color-card-border)] bg-[var(--color-card-surface)]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md">
      <ul className="mx-auto grid max-w-lg grid-cols-5 gap-1 px-1.5 pb-1 pt-2 sm:px-2">
          ...
          return (
            <li key={href} className="min-w-0">
              <Link
                href={href}
                className={clsx(
                  "flex min-w-0 flex-col items-center gap-0.5 rounded-lg px-0.5 py-1.5 transition-all duration-200 sm:px-1",
                  isActive
              ...
              >
                ...
                <span className="max-w-full truncate text-[9px] font-bold tracking-tight sm:text-[10px]">{t(labelKey)}</span>
              </Link>
            </li>
          );
        })}
      </ul>
      <p className="hidden pb-1 text-center text-[9px] tracking-wider text-[var(--color-text-muted)] opacity-40 sm:block">
        v{pkg.version}
      </p>
    </nav>
  );
